...
# to fix some error link: https://stackoverflow.com/questions/20554074/sklearn-omp-error-15-initializing-libiomp5md-dll-but-found-mk2iomp5md-dll-a
...
def detect(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    (save_dir / 'half_sec_frames').mkdir(parents=True, exist_ok=True) # make the half_sec_frames dir
    # f"{save_dir}/half_sec_frames".mkdir(parents=True, exist_ok=True)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # this is to save the frames to a folder every 1/2 second 
            # I need to change the save folder to the runs/detect/exp folder instead - done
            # I need to only save frames with labels detected - tbc
